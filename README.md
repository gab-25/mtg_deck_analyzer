...
- **Multi-language card content**: Fetches card names and descriptions in the chosen language (English, Italian, Spanish, French, German — defined in a single registry in `constants.py`). Decklists can also be **pasted in any of these languages**: card names written in the chosen language (e.g. Italian `Fulmine` for `Lightning Bolt`) are resolved back to the card via Scryfall's localized printed-name search, so a non-English decklist no longer fails to resolve. If a card is not available in the chosen language it falls back intelligently: first to an alternative set that has it localized, then to a Gemini machine translation, and finally to the English text. Each card records its text **provenance** (`official` / `machine` / `english`), surfaced as an "Auto-translated" or "English text" badge in the web page and a note in the PDF, so machine-translated rules text is never passed off as official. The interface itself stays in English.
