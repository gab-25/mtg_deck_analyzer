...
- **Deck ownership**: A deck belongs to the user who submitted it, and to nobody else. Your library shows your decks only, and opening, editing, re-analyzing and deleting are all the same permission — another user's deck answers 404 rather than confirming it exists. Decks created before ownership existed stay ownerless and keep behaving exactly as they did, available to every signed-in user. Deleting an account deletes its decks and their version history along with it — this is irreversible, so it never leaves those decks ownerless and readable by everyone instead.
...
├── access.py          # Deck ownership rules (one source of truth)
