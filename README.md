...
A powerful Python tool for Magic: The Gathering that, given a textual card list as input, fetches card images and descriptions in real time through the **Scryfall** API and produces a strategic deck analysis generated by **Google Gemini** via the official `google-genai` SDK.

It works two ways from the same analysis engine:

- a **CLI** that generates a professional PDF document with card images, descriptions and the strategy guide, and
- a **web service** (FastAPI + Postgres + HTMX/DaisyUI) that lets you paste a decklist in the browser, view the report as a web page and download the same PDF (see [Web Service](#web-service)).
...
- **Web Service**: Exposes the same engine as a FastAPI app with a Postgres database and HTMX/DaisyUI pages — submit decklists, browse previously analyzed decks, and download their PDF (see [Web Service](#web-service)).
